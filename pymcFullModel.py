...
## converting the numbers solved here back to real time (scaled here for speed, introduces minor error)
## to get the right advection:reaction, consider the velocity * 10 and the mass is put in *100 on the stochastic, so a_r(carbon*100*10) gives the right carbon A_R for Ma
...
CaInject=pm.Uniform('CaInject',-2.0,-0.9)
...
obs1 = pm.Normal("ACd13c", predACd13c, errC, value=np.array(LVd13c.values), observed=True)
obs2 = pm.Normal("ACd44ca", predACd44ca, errCa, value=np.array(LVd44ca.values), observed=True)
#@pm.deterministic
...
model = pm.Model([errC,errCa,predACd44ca,predACd13c,obs1,obs2,carbonMass,caMass,CaInject,caOffset])
model = pm.Model([errC,errCa,predACd44ca,predACd13c,obs1,obs2,carbonMass,caMass,CaInject,caOffset])
...
with plt.style.context('fivethirtyeight'):
    ...
    def a_r(carbon):
        return(mesh.dt*carbon*.1*mesh.u.max())/(mesh.massRatio[0][0]*mesh.dt*mesh.r)
    ...
    A.plot(range(N),(mcmc.trace('caMass')[:]),lw=1)
    B.plot(range(N),(mcmc.trace('CaInject')[:]),lw=1)
    C.plot(range(N),(mcmc.trace('errC')[:]),lw=1)
    D.plot(range(N),a_r(mcmc.trace('carbonMass')[:]*100),lw=1)
    E.plot(range(N),(mcmc.trace('errCa')[:]),lw=1)

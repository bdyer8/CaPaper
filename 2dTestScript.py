...
def flowMatrix(self,phase,parameters):  #positive('fluid','d13c',x)
        ...
        for k in range(len(parameters)):  #k through trackable properties, d13c, d18o, etc
            A=self.printBox(phase,parameters[k])
            matrix[k][1][self.positiveX]=np.roll(A*(np.roll(self.positiveX,1)),-1)[self.positiveX]
            matrix[k][1][~self.positiveX]=np.roll(A*(np.roll(~self.positiveX,-1)),1)[~self.positiveX]
            matrix[k][2][self.positiveY]=np.roll(A*(np.roll(self.positiveY,1,0)),-1,0)[self.positiveY]
            matrix[k][2][~self.positiveY]=np.roll(A*(np.roll(~self.positiveY,-1,0)),1,0)[~self.positiveY]
            matrix[k][0][self.zeroSum]=A[self.zeroSum]
            matrix[k][0][~self.zeroSum]=map(newFluid,matrix[k][1][~self.zeroSum],np.abs(self.u[~self.zeroSum]),
                                            matrix[k][2][~self.zeroSum],np.abs(self.v[~self.zeroSum])) 
...
#ani.save('compPlot_r5_200_variableA.mp4', dpi=150, writer = FFwriter, fps=30, extra_args=['-vcodec', 'libx264'])

...
mR=[3.23,6.75]
bR=[-23.4,12.2]
yData=mR*xData+bR+np.random.normal(0,1,(100,2))
...
mcmc=pm.MCMC(model)
#%%
mcmc.sample(50000,25000,2)

#%%
for i in range(2):
    colors=['r','b']
    plt.plot(np.sort(x.value[:,i]),sorted(predY.stats()['quantiles'][50][:,i]),color=colors[i],lw=2);
    plt.scatter(xData[:,i],yData[:,i],color=colors[i]);
    plt.plot(np.sort(x.value[:,i]),sorted(predY.stats()['quantiles'][2.5][:,i]),color=colors[i],lw=2,alpha=.6);
    plt.plot(np.sort(x.value[:,i]),sorted(predY.stats()['quantiles'][97.5][:,i]),color=colors[i],lw=2,alpha=.6);
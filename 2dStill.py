...
im = axarr0.imshow(a, cmap=plt.get_cmap('nipy_spectral'), vmin=-5, vmax=5)
#im = axarr0.imshow(a, cmap=plt.get_cmap('prism'), vmin=-5, vmax=5)
axarr1.set_xlim([-7, 6])
axarr1.set_ylim([75, 97])
axarr0.set_xlim([1, 99])
axarr0.set_ylim([25, 1])
...
#v=load('vNew2.npy')
#v[np.abs(v)<.001]=0
u=load('uLensMask.npy')
#u[np.abs(u)<.001]=0
v=load('vLensMask.npy')
#v[np.abs(v)<.001]=0
...
#qui = axarr0.quiver(X[::2,::2],Y[::2,::2],u[::2,::2],v[::2,::2],pivot='mid')
axarr0.plot(7*np.ones(c[3:,25].size),np.linspace(0,100,c[3:,5].size), lw=2)
axarr0.plot(20*np.ones(c[3:,25].size),np.linspace(0,100,c[3:,5].size), lw=2)
axarr0.plot(35*np.ones(c[3:,25].size),np.linspace(0,100,c[3:,5].size), lw=2)
...
#            d[23:27,47:53]=-6
    ...
    c2=c
    im.set_array(a2)
    line.set_data(np.array(a2[:,7]),np.array(list(reversed(range(0,a2[:,4].size)))))
    line2.set_data(np.array(a2[:,20]),np.array(list(reversed(range(0,c2[:,4].size)))))
    line3.set_data(np.array(a2[:,35]),np.array(list(reversed(range(0,a2[:,4].size)))))
...
ani.save('fluidFlowCarboniferous5.mp4', writer = FFwriter, fps=30, extra_args=['-vcodec', 'libx264'])
